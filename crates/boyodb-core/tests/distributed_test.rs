        ...
        loop {
            ...
            tokio::spawn(async move {
                ...
                if let Ok(Req::ExecuteSubQuery { plan_json }) = serde_json::from_slice::<Req>(&buf)
                {
                    ...
                    if let Ok(local_plan) = serde_json::from_value::<
                        boyodb_core::planner_distributed::LocalPlan,
                    >(plan_json)
                    {
                        let result = db.execute_distributed_plan(local_plan.kind);
                        if let Ok(ref res) = result {
                            println!(
                                "MockServer[{}]: SUCCESS, {} bytes ipc",
                                name,
                                res.records_ipc.len()
                            );
                        } else {
                            ...
                            Ok(qr) => {
                                ...
                                if let Some(obj) = val.as_object_mut() {
                                    ...
                                    if let Some(ipc_val) = obj.remove("records_ipc") {
                                        let bytes: Vec<u8> =
                                            serde_json::from_value(ipc_val).unwrap();
                                        use base64::Engine;
                                        let b64 =
...
async fn test_distributed_query_broadcast() {
    ...
    println!(
        "Test: Ports - rpc1={}, gossip1={}, rpc2={}, gossip2={}",
    ...
    tokio::spawn(boyodb_core::cluster::start_gossip_listener(
        gossip1,
        cm1.clone(),
    ));
    tokio::spawn(boyodb_core::cluster::start_gossip_sender(
        _rx1,
        cm1.shutdown_flag(),
    ));
    ...
    for attempt in 0..20 {
        ...
        println!(
            "Test: Convergence attempt {} - node1={}, node2={}",
            attempt, g1_count, g2_count
        );
    ...
    assert!(!resp.records_ipc.is_empty(), "Should have results");
    ...
    println!(
        "Test: COUNT(*) response has {} bytes",
        agg_resp.records_ipc.len()
    );
