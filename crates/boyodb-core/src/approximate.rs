...
impl TDigest {
    ...
    fn compress(&mut self) {
        ...
        for centroid in self.centroids.iter().skip(1) {
            ...
            let q = (current.weight / 2.0
                + compressed.iter().map(|c: &Centroid| c.weight).sum::<f64>())
                / self.total_weight;
...
pub fn approx_top_k<T: Hash + Clone + Eq>(
...
mod tests {
    ...
    fn test_hyperloglog() {
        ...
        assert!(
            count > 9800 && count < 10200,
            "HLL count {} not within expected range",
            count
        );
    }
    ...
    fn test_tdigest_quantiles() {
        ...
        assert!(
            p50 > 400.0 && p50 < 600.0,
            "P50 {} not in expected range 400-600",
            p50
        );
        ...
        assert!(
            ...
            "P99 {} not in expected range 900-1000",
            p99
        );
    }
