...
async fn test_pg_wire_simple_query() {
    ...
    let mut server = Command::new(binary_path)
        .args([data_path, &bind_addr, "--pg-port", &pg_port.to_string()])
    ...
    for _ in 0..20 {
        ...
        // Try connecting
        if let Ok(mut stream) = TcpStream::connect(format!("127.0.0.1:{}", pg_port)).await {
            ...
            assert_eq!(
                resp_buf[0], b'T',
                "Expected RowDescription ('T') for SELECT 1"
            );
...
async fn test_pg_wire_transaction_stub() {
    ...
    for _ in 0..20 {
        ...
        if let Ok(mut stream) = TcpStream::connect(format!("127.0.0.1:{}", pg_port)).await {
            ...
            let n = stream.read(&mut resp_buf).await.unwrap();
            if n > 0 && resp_buf[0] == b'R' && n >= 9 && resp_buf[8] == 3 {
                let password = "boyo";
                let mut p_buf = Vec::new();
                p_buf.push(b'p');
                let p_len = 4 + password.len() + 1;
...
async fn test_pg_wire_introspection_queries() {
    ...
    for _ in 0..20 {
        ...
        if let Ok(mut stream) = TcpStream::connect(format!("127.0.0.1:{}", pg_port)).await {
            ...
            // Read all response data - might be split across multiple packets
            let mut total_read = 0;
            loop {
                let n = stream.read(&mut resp_buf[total_read..]).await.unwrap();
                if n == 0 {
                    break;
                }
                total_read += n;
                // Check if we've received CommandComplete ('C') or ReadyForQuery ('Z')
                // which indicates end of response
                let response_bytes = &resp_buf[..total_read];
                if response_bytes.contains(&b'Z') || response_bytes.contains(&b'C') {
                    break;
                }
                // Safety limit
                if total_read > 4096 {
            ...
            println!("Received version() response: {:?}", &resp_buf[..total_read]);
            if resp_buf[0] == b'E' {
                panic!(
                    "Received ErrorResponse for SELECT version(): {:?}",
                    &resp_buf[..total_read]
                );
            }
            ...
            assert_eq!(
                resp_buf[0], b'T',
                "Expected RowDescription ('T') for SELECT version()"
            );
            // Check response contains "BoyoDB"
            let response_str = String::from_utf8_lossy(&resp_buf[..total_read]);
            assert!(
                response_str.contains("BoyoDB"),
                "Expected BoyoDB in version response, got: {}",
                response_str
            );
            ...
            let n = stream.read(&mut resp_buf).await.unwrap();
            println!(
                "Received SHOW server_version response: {:?}",
                &resp_buf[..n]
            );
            if resp_buf[0] == b'E' {
                panic!(
                    "Received ErrorResponse for SHOW server_version: {:?}",
                    &resp_buf[..n]
                );
            }
            assert_eq!(
                resp_buf[0], b'T',
                "Expected RowDescription ('T') for SHOW server_version"
            );
    ...
    assert!(
        connected,
        "Failed to connect and verify introspection queries"
    );
}
