...
async fn test_pg_wire_extended_query() {
    ...
    for _ in 0..20 {
        ...
        if let Ok(mut stream) = TcpStream::connect(format!("127.0.0.1:{}", pg_port)).await {
            ...
            p_content.extend_from_slice(stmt_name.as_bytes());
            p_content.push(0);
            p_content.extend_from_slice(query.as_bytes());
            p_content.push(0);
            ...
            b_content.extend_from_slice(stmt_name.as_bytes());
            b_content.push(0);
            ...
            println!(
                "Extended Query Response Total {} bytes: {:?}",
                accumulated_resp.len(),
                accumulated_resp
            );
